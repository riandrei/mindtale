              ...
              <>
                <div className={styles.Middle_top}>
                  <div className={styles.Middle_top_inner}>
                    <span>{result.word.charAt(0).toUpperCase() + result.word.slice(1)}</span>
                    {result.phonetics[0]?.audio && (
                      <img src={Sound} alt="Play sound" onClick={playAudio} />
                    )}
                  </div>
                  <span style={{color: "black"}}>{`Tagalog Translation: ${translatedWord}`}</span>
                  <span className={styles.Speech}>{result.meanings[0]?.partOfSpeech}</span>
                </div>
